...
public class Inventory : NetworkBehaviour
{
    ...
    void PickUp() {
        if(Physics.SphereCast(view.position, grabWidth, view.forward, out RaycastHit hit, grabDistance, itemLayer.value)) {
            ...
            if(!Input.GetButtonDown("Interact")) {
                hit.transform.GetComponent<OldItem>().hovered = true;
                return;
            } 
            GameObject selectedItem = hit.transform.gameObject;
            int slot = selectedItem.GetComponent<OldItem>().data.type;

            // print(hit.transform.name); 
            // print(selectedItem.GetComponent<OldItem>().data.type);

            if(inventory[slot] != null) return;

            selectedItem.GetComponent<OldItem>().ItemPickupServerRpc(false, Vector3.zero, Vector3.zero);
            inventory[slot] = selectedItem;
            selectedItem.GetComponent<OldItem>().model.SetActive(false);

            GameObject playerItem = Instantiate(selectedItem.GetComponent<OldItem>().ghostItemPrefab, gunHolder); 
    ...
    void Drop(int i, bool thrown) {
            ...
            itemReal.GetComponent<OldItem>().ItemPickupServerRpc(true, thrown ? (view.forward * throwStength) : Vector3.zero, GetComponent<Rigidbody>().linearVelocity);
            itemReal.GetComponent<OldItem>().UpdateAmmoServerRpc(itemReal.GetComponent<OldItem>().ammo);
    ...
    void select(int weaponIndex) {
        ...
        for (int i = 0; i < inventory.Length; i++)
        {
            if(inventory[i] != null) {
                ...
                inventory[i].GetComponent<OldItem>().SelectServerRpc(i == weaponIndex);
            }
        }
    }
    ...
    IEnumerator GhostDelay() {
        ...
        Destroy(itemGhost);
        itemReal.GetComponent<OldItem>().model.SetActive(true);
        itemGhost = null;
        itemReal = null;
    }
